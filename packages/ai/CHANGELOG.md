...
- Preserve provider-stated cooldown deadlines across processes while allowing complete usage reports to replace older heuristic blocks before selection after a restart; conservatively retain deadlines written by legacy executables.
- Persist complete usage-report reset deadlines in place of the current request's heuristic cooldown, allowing subscription credentials to recover after shorter authoritative resets.
